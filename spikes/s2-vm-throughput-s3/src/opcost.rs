...
/// The clock this build runs at, for turning nanoseconds into cycles.
///
/// An S3's maximum is 240 MHz where a C3's is 160, and the difference is the
/// whole reason to run this on both.
pub const CLOCK_MHZ: u64 = 240;
...
fn dispatch_cost() {
    ...
    if n_hi > n_lo && t_hi > t_lo {
        ...
        // Derived from the clock the chip is actually running at rather
        // than a number written into the string. The C3 build said "at
        // 160 MHz" and the S3 inherited it, which would have reported 93
        // cycles for a chip doing 140.
        println!(
            "   that is {} cycles per instruction at {CLOCK_MHZ} MHz",
            per_instruction_centi * CLOCK_MHZ / 100_000
        );
    }
}
